...
describe('View', () => {
	...
	for (const { label, options, expected } of [
		...
		it(`it uses a ${label} width for the view container element`, () => {
			...
			const waveContainer = view.container.querySelector<HTMLDivElement>(
				'.waveplayer-waveform-container',
			);
	...
	it('gets and sets the container element', () => {
		document.body.innerHTML =
			'<div id="first-container"></div><div id="second-container"></div>';
	...
	it('draws the correct number of bars on the canvas', () => {
		...
		Object.defineProperty(
			document.body.querySelector<HTMLDivElement>('#container'),
			'clientWidth',
			{
				value: view.width,
				configurable: true,
		...
		Object.defineProperty(waveContainer, 'clientWidth', {
			value: view.width,
			configurable: true,
		});
		...
		expect(spy).toHaveBeenCalledTimes(
			Math.floor(view.width / (view.barWidth + view.barGap)),
		);
