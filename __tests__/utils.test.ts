...
describe('utils', () => {
	...
	describe('rgb2hsv', () => {
		for (const { label, rgb, hsv } of [
			{
				label: 'lila',
				rgb: { r: 201, g: 100, b: 200 },
				hsv: { h: 301, s: 50, v: 79 },
			},
			{
				label: 'green',
				rgb: { r: 100, g: 200, b: 100 },
				hsv: { h: 120, s: 50, v: 78 },
			},
	...
	describe('getJson', () => {
		it('successfully fetches and parses a JSON file', async () => {
			...
			await expect(getJson<number[]>('/sample.json')).resolves.toEqual([
				1, 2, 3,
			]);
		...
		it('fails to fetch and parses a JSON file', async () => {
			...
			await expect(getJson<number[]>('/sample.json')).rejects.toBeInstanceOf(
				HttpError,
			);
			expect(mockOpen).toHaveBeenCalledWith('GET', '/sample.json');
			expect(mockSend).toHaveBeenCalled();
