 ...
 * @returns {boolean}
 */
const isWebAudioStrategy = (
	strategy: Strategy,
): strategy is WebAudioStrategy => {
...
class Player implements PlayerContract {
	 ...
	 * @var {PlayerOptions}
	 */
	private static _defaultOptions: Readonly<
		Omit<PlayerOptions, 'audioElement'>
	> = {
		preload: 'metadata',
	};
	...
	public async load(url: string, strategy: Strategy): Promise<this> {
		await Promise.all<this>([
			this.loadAudio(url),
			this.loadWaveform(url, strategy),
		]);
			...
			this._errorHandler = (e: Event): void => {
				...
				switch (error.code) {
					...
					case error.MEDIA_ERR_NETWORK:
						reject(
							new Error('There was a problem downloading the audio file.'),
						);
						return;
					...
					case error.MEDIA_ERR_SRC_NOT_SUPPORTED:
						reject(new Error('The audio file is not supported.'));
						return;
					default:
	...
	private async resolveData(
		...
		const data =
			cache && this.cachedDataExists(key)
				? this.parseCachedData(key)
				: await callback();
