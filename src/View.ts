...
class View implements ViewContract {
	...
	private computeBarCoordinates(
		...
		for (
			let i = 0, j = 0;
			j + totalBarWidth < this._data.length;
			i += totalBarWidth, j += incr
		) {
			x.push(i);
	...
	private drawBars(x: number[], y: number[], norm: number): void {
		...
		while (x[i] < progressCoordinate) {
			...
			const progresIndicatorColor =
				this.createProgressIndicatorColorVariation(incr);
	...
	private drawBar(
		...
		context.fillRect(
			x,
			(this._canvas.height - barHeight) / 2,
	...
	private createGradient(
		context: CanvasRenderingContext2D,
		colorVariation: [RgbColor, RgbColor],
	): CanvasGradient {
		...
		gradient.addColorStop(
			0.3,
			`rgba(${Object.values(colorVariation[0]).join(', ')}, 1)`,
		);
