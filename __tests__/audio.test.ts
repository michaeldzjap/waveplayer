...
describe('audio', () => {
	...
	describe('extractAmplitudes', () => {
		it('extracts the amplitudes from a mono audio file', async () => {
			...
			const { mockDecodeAudioData, mockGetChannelData } = mockWebAudio(1, [
				sine,
			]);
		...
		it('extracts the amplitudes from a stereo audio file', async () => {
			...
			const { mockDecodeAudioData, mockGetChannelData } = mockWebAudio(
				2,
				noise,
			);

			const data = await extractAmplitudes('/noise.wav', {
				points: 256,
				logarithmic: true,
			});
